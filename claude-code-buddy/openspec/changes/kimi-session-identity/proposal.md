...
- **Kimi pane 聚焦**：`CmuxClient` 新增 `focus_by_kimi_sid(sid)`：从 `~/.kimi-code/sessions/wd_<dir>_<hash>/session_<uuid>/` 目录名解析 sid → 工作目录，匹配 cmux pane 的 `requested_working_directory` 聚焦；`bridge.py` 的 `_select_session` 在现有四路匹配后追加该回退。
