...
class ResultProcessor:
    ...
    async def _optimize_text_pipeline(self, text: str) -> str:
        try:
            from text_optimizer import optimize_text

            return await optimize_text(text)
        except Exception as e:
            logger.warning(f"text_optimizer 处理失败，使用原文: {e}")
            return text
    ...
    async def _handle_message(self, message: str) -> None:
        ...
        if self.state.voice_hud and text.strip():
            self.state.voice_hud.hide()
