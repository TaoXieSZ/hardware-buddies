...
def _resolve_lib_dir() -> Path:
    """Resolve llama native library directory for source and frozen builds."""
    module_bin = Path(__file__).resolve().parent / "bin"
    candidates = [module_bin]

    if getattr(sys, "frozen", False):
        exe_dir = Path(sys.executable).resolve().parent
        candidates.append(exe_dir / "util" / "fun_asr_gguf" / "bin")
        meipass = getattr(sys, "_MEIPASS", None)
        if meipass:
            candidates.append(Path(meipass) / "util" / "fun_asr_gguf" / "bin")
