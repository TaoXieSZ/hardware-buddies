...
# 客户端配置
class ClientConfig:
    addr = 'localhost'           # Server 地址
    port = '6016'               # Server 端口

    # 快捷键配置列表
    shortcuts = [
        {
            'key': 'f18',           # 产品层统一名称为 F18；设备配置层保存 HID Usage ID 0x6D；Windows 运行时监听 VK_F18（0x81）
        ...
        {
            'key': 'numpad_subtract',  # 小键盘减号也可触发语音
            'type': 'keyboard',
            'suppress': True,
            'hold_mode': True,
            'enabled': True
...
示例配置：
  {'key': 'f18', 'type': 'keyboard', 'suppress': True, 'hold_mode': True, 'enabled': True}, 
