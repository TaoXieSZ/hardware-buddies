...
class ShortcutManager:
    ...
    def __init__(self, state: 'ClientState', shortcuts: List['Shortcut']):
        ...
        self._audio_cue_key_down: Dict[str, bool] = {}
        self._audio_cue_longpress_triggered: Dict[str, bool] = {}
        self._audio_cue_press_token: Dict[str, int] = {}
        # 语音热键提示音参数（可按需自定义）
        self._audio_cue_press_freq = 880
        self._audio_cue_release_freq = 660
        self._audio_cue_press_ms = 120
        self._audio_cue_release_ms = 120
        self._audio_cue_enabled = True
        self._audio_cue_config_mtime_ns: Optional[int] = None

        # 初始化快捷键任务
        self._init_tasks()
        self._reload_audio_cue_enabled(force=True)
    ...
    def create_keyboard_filter(self):
        ...
        def win32_event_filter(msg, data):
            ...
            if msg in KEY_DOWN_MESSAGES:
                self._handle_audio_cue_keydown(key_name, task.threshold)
                self._event_handler.handle_keydown(key_name, task)
            elif msg in KEY_UP_MESSAGES:
                self._handle_audio_cue_keyup(key_name)
    ...
    def _handle_audio_cue_keydown(self, key_name: str, threshold: float) -> None:
        if self._audio_cue_key_down.get(key_name):
            return
        self._audio_cue_key_down[key_name] = True
        self._audio_cue_longpress_triggered[key_name] = False
        token = self._audio_cue_press_token.get(key_name, 0) + 1
        self._audio_cue_press_token[key_name] = token
        self._pool.submit(self._trigger_audio_cue_longpress_beep, key_name, token, threshold)

    def _handle_audio_cue_keyup(self, key_name: str) -> None:
        if self._audio_cue_key_down.get(key_name) and self._audio_cue_longpress_triggered.get(key_name):
            self._pool.submit(self._play_audio_cue, 'up')
        self._audio_cue_key_down[key_name] = False
        self._audio_cue_longpress_triggered[key_name] = False
        self._audio_cue_press_token[key_name] = self._audio_cue_press_token.get(key_name, 0) + 1

    def _play_audio_cue(self, phase: str = 'down') -> None:
        """播放平和提示音。phase: down | up"""
        self._reload_audio_cue_enabled()
        if not self._audio_cue_enabled:
            return
        is_down = (phase == 'down')
        freq = self._audio_cue_press_freq if is_down else self._audio_cue_release_freq
        duration_ms = self._audio_cue_press_ms if is_down else self._audio_cue_release_ms
        # 保护参数范围，避免异常输入导致报错
        freq = max(37, min(int(freq), 32767))
        ...
        try:
            ...
            logger.debug("failed to play voice hotkey audio cue")

    @staticmethod
    def _shared_config_path() -> Path:
        raw = os.environ.get("CAPSWRITER_SHARED_CONFIG", "").strip()
        if raw:
            return Path(raw)
        return Path(tempfile.gettempdir()) / "capswriter_config.json"

    def _reload_audio_cue_enabled(self, force: bool = False) -> None:
        path = self._shared_config_path()
        try:
            stat = path.stat()
        except OSError:
            if force:
                self._audio_cue_enabled = True
                self._audio_cue_config_mtime_ns = None
            return

        mtime_ns = stat.st_mtime_ns
        if not force and mtime_ns == self._audio_cue_config_mtime_ns:
            return
        ...
        value = payload.get("enable_audio_cue")
        if isinstance(value, bool):
            self._audio_cue_enabled = value
            self._audio_cue_config_mtime_ns = mtime_ns

    def _trigger_audio_cue_longpress_beep(self, key_name: str, press_token: int, threshold: float) -> None:
        """Only play audio cue when press duration reaches threshold."""
        time.sleep(max(0.0, threshold))
        if (
            self._audio_cue_key_down.get(key_name)
            and not self._audio_cue_longpress_triggered.get(key_name)
            and press_token == self._audio_cue_press_token.get(key_name)
        ):
            self._audio_cue_longpress_triggered[key_name] = True
            self._play_audio_cue('down')
